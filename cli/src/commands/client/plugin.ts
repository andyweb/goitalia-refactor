    ...
    plugin
      ...
      .description(
        "Install a plugin from a local path or npm package.\n" +
          "  Examples:\n" +
          "    goitalia plugin install ./my-plugin              # local path\n" +
          "    goitalia plugin install @acme/plugin-linear      # npm package\n" +
          "    goitalia plugin install @acme/plugin-linear@1.2  # pinned version",
      )
          ...
          for (const ex of rows) {
            console.log(
              `${pc.bold(ex.displayName)}  ${pc.dim(ex.pluginKey)}\n` +
                `  ${ex.description}\n` +
                `  ${pc.cyan(`goitalia plugin install ${ex.localPath}`)}`,
            );
          }
        } catch (err) {
          handleCommandError(err);
