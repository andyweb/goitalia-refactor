...
function defaultSecretsKeyFilePath(): string {
  return resolveDefaultSecretsKeyFilePath(resolveGoItaliaInstanceId());
}
function defaultStorageBaseDir(): string {
  return resolveDefaultStorageDir(resolveGoItaliaInstanceId());
}
...
export async function envCommand(opts: { config?: string }): Promise<void> {
  ...
  let config: GoItaliaConfig | null = null;
...
function collectDeploymentEnvRows(config: GoItaliaConfig | null, configPath: string): EnvVarRow[] {
