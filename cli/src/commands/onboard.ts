...
type OnboardDefaults = Pick<GoItaliaConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;
...
function quickstartDefaultsFromEnv(): {
  ...
  const instanceId = resolveGoItaliaInstanceId();
...
function canCreateBootstrapInviteImmediately(config: Pick<GoItaliaConfig, "database" | "server">): boolean {
  return config.server.deploymentMode === "authenticated" && config.database.mode !== "embedded-postgres";
}

export async function onboard(opts: OnboardOptions): Promise<void> {
  printGoItaliaCliBanner();
  p.intro(pc.bgCyan(pc.black(" goitalia onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveGoItaliaInstanceId());
  p.log.message(
    pc.dim(
  ...
  let llm: GoItaliaConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@goitalia/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `goitalia doctor`"));
      }
    }
  ...
  p.note(
    [
      `Run: ${pc.cyan("goitalia run")}`,
      `Reconfigure later: ${pc.cyan("goitalia configure")}`,
      `Diagnose setup: ${pc.cyan("goitalia doctor")}`,
    ].join("\n"),
    "Next commands",
  );
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("goitalia run")}`,
        `Then: ${pc.cyan("goitalia auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
