...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    "# GoItalia environment variables",
    "# Generated by GoItalia CLI commands",
...
export function resolveGoItaliaEnvFile(configPath?: string): string {
...
export function loadGoItaliaEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeGoItaliaEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readGoItaliaEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeGoItaliaEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeGoItaliaEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readGoItaliaEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeGoItaliaEnvEntries(next, filePath);
  return next;
}
