...
const DATA_DIR_OPTION_HELP =
  "GoItalia data directory root (isolates state from ~/.paperclip)";

program
  .name("goitalia")
  .description("GoItalia CLI — setup, diagnose, and configure your instance")
  .version("0.2.7");
...
program.hook("preAction", (_thisCommand, actionCommand) => {
  ...
  loadGoItaliaEnvFile(options.config);
});
...
program
  ...
  .option("--run", "Start GoItalia immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your GoItalia setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run GoItalia")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the GoItalia server API")
