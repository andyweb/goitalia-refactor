  ...
  const packageJson = {
    ...
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@goitalia/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@goitalia/plugin-sdk": sdkDependency,
...
import { createPluginBundlerPresets } from "@goitalia/plugin-sdk/bundlers";
...
`,
  ...
  writeFile(
    ...
    `import { definePlugin, runWorker } from "@goitalia/plugin-sdk";
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@goitalia/plugin-sdk/ui";
...
import { createTestHarness } from "@goitalia/plugin-sdk/testing";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@goitalia/plugin-sdk\` and \`@goitalia/shared\` from a local Paperclip checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.paperclip-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
- \`pnpm build\` uses esbuild presets from \`@goitalia/plugin-sdk/bundlers\`.
