...
// Load .env from repo root so DATABASE_URL is available before spawning children
const __repoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const __dotenvPath = path.join(__repoRoot, ".env");
if (existsSync(__dotenvPath)) {
  for (const line of readFileSync(__dotenvPath, "utf8").split("\n")) {
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@goitalia/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[paperclip] Command failed with code ${status.code}: pnpm --filter @goitalia/db exec tsx src/migration-status.ts --json\n`,
    );
...
async function buildPluginSdk() {
  ...
  const result = await runPnpm(
    ["--filter", "@goitalia/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@goitalia/server", serverScript, ...forwardedArgs],
