...
import type { CompanyPortabilityFileEntry } from "@goitalia/shared";
...
describe("company portability", () => {
  const paperclipKey = "goitalia/paperclip/paperclip";
    ...
    const companySkills = [
      {
        ...
        slug: "paperclip",
        name: "paperclip",
        description: "GoItalia coordination skill",
        markdown: "---\nname: paperclip\ndescription: Paperclip coordination skill\n---\n\n# Paperclip\n",
        sourceType: "github",
        sourceLocator: "https://github.com/goitalia/paperclip/tree/master/skills/paperclip",
  ...
  it("exports referenced skills as stubs by default with sanitized Paperclip extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "GoItalia"');
    ...
    expect(asTextFile(exported.files["skills/goitalia/paperclip/paperclip/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/goitalia/paperclip/paperclip/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/goitalia/paperclip/paperclip/references/api.md"]).toBeUndefined();
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/goitalia/paperclip/paperclip/SKILL.md"])).toContain("# Paperclip");
    expect(asTextFile(exported.files["skills/goitalia/paperclip/paperclip/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/goitalia/paperclip/paperclip/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/goitalia/paperclip/paperclip/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/goitalia/paperclip/paperclip/SKILL.md"]).toBeDefined();
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-paperclip",
        companyId: "company-1",
        key: "goitalia/paperclip/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/goitalia/paperclip/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
    ...
    expect(asTextFile(exported.files["skills/goitalia/paperclip/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/goitalia/paperclip/release-changelog/SKILL.md"])).toContain("goitalia/paperclip/release-changelog");
  });
