...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@goitalia/db";
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@goitalia/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
