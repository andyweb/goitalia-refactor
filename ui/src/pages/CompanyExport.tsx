 ...
 * renderYamlBlock.
 */
function filterGoItaliaYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm goitalia company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [GoItalia](https://paperclip.ing) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [GoItalia](https://paperclip.ing) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  const effectiveFiles = useMemo(() => {
    ...
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterGoItaliaYaml(exportData.files[yamlPath], checkedFiles);
    }
