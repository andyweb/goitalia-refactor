...
export function PluginManager() {
  ...
  const agentBtn = (msg: string) => (
    <a href={"/" + (selectedCompany?.issuePrefix || "") + "/chat?msg=" + encodeURIComponent(msg)} onClick={() => { if (selectedCompany?.id) fetch("/api/onboarding/onboarding-step", { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify({ companyId: selectedCompany.id, step: 99 }) }); }} className="text-xs px-3 py-1.5 rounded-lg transition-all no-underline" style={{ background: "linear-gradient(135deg, hsl(158 64% 42% / 0.15), hsl(158 64% 42% / 0.08))", border: "1px solid hsl(158 64% 42% / 0.25)", color: "rgba(255,255,255,0.7)" }}>Crea agente</a>
  );
